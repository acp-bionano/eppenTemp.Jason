/* 
  Charles Fracchia  1 Aug 2014
  Version that uses 9bit resolution of temperature
  Modified from: http://www.netfluvia.org/code/ds18B20_test.pde
*/
...
void loop() {
  ...
  tinySerial.print("R=");
  for( i = 0; i < 8; i++) {
    tinySerial.print(addr[i], HEX);
    tinySerial.print(" ");
  }
#endif

  if ( OneWire::crc8( addr, 7) != addr[7]) {
      tinySerial.print("CRC is not valid!\n");
      return;
  }

  if ( addr[0] != 0x28) {
      tinySerial.print("Device is not a DS18B20 family device.\n");
      return;
  }
  ...
  if (!present) {
    tinySerial.print("ERROR: selected device not present\n");
    return;
  }
  ...
  for ( i = 0; i < 9; i++) {           // we need 9 bytes
    data[i] = ds.read();
#ifdef DEBUG
    tinySerial.print(data[i], HEX);
    tinySerial.print(" ");
#endif
  }

  if (data[8] != OneWire::crc8(data,8)) {
    tinySerial.print("ERROR: CRC didn't match\n");
    return;
  }
  
  /* print raw bytes from which we'll extract temp data */
#ifdef DEBUG 
  tinySerial.print("[MSB:");
  tinySerial.print(data[TEMP_MSB],BIN);
  tinySerial.print(" LSB:");
  tinySerial.print(data[TEMP_LSB],BIN);
  tinySerial.print("] ");
#endif
...
#ifdef DEBUG
  tinySerial.print(temp_c_int,DEC);
#endif
...
#ifdef DEBUG
  tinySerial.print(" ");
  tinySerial.print(temp_c_frac * 10000,DEC);
#endif
  ...
  tinySerial.print("R=");
  tinySerial.print(temp_c,DEC); // tinySerial.print truncates after the decimal pt
  //tinySerial.print(" F=");
  //tinySerial.print(temp_f,DEC);
